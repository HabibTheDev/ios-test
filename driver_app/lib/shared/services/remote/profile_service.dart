...
class ProfileService extends ApiClient implements ProfileRepo {
  ...
  Future<List<CountryCodeModel>?> getCountries() async {
    try {
      final response = await getRequest(path: ApiEndpoint.getCountries, isTokenRequired: false);
  ...
  Future<CustomerModel?> getCustomerDetails() async {
    try {
      ...
      return customerModelFromJson(jsonEncode(response.data['data']));
    } on ApiException catch (error) {
      showToast(error.message ?? AppString.unknownError);
      debugPrint(error.message ?? AppString.unknownError);
    } catch (error) {
  ...
  @override
  Future<bool> updateCustomerProfile({String? filePath, required Map<String, dynamic> body}) async {
    try {
      final response = await multipartPutRequest(
        path: ApiEndpoint.updateOwnProfile,
        filePath: filePath,
        body: body,
      );
