...
class HandoverController extends GetxController {
  HandoverController({required this.singleTaskRepo});
  ...
  void stepActionOnTap({required Steps? step, required AppLocalizations? locale}) async {
      ...
      await Get.toNamed(
        ...
        arguments: {
          ArgumentsKey.inspectionArgumentsModel: InspectionArgumentsModel(
            title: '${locale?.departureInspection}',
            returnScreen: RouterPaths.handover,
            taskId: taskModel.value?.task?.id,
            carID: taskModel.value?.task?.carId,
            contactID: taskModel.value?.task?.contactId,
            taskStepId: step?.id,
            inspectionType: InspectionTypeEnum.departureInspection,
    ...
    stepLoading(true);
    final result = await singleTaskRepo.completeHandoverStep(taskId: taskModel.value?.task?.id, stepId: step?.id);
